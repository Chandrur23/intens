...
if response.status_code == 200:
    ...
    for forecast in forecasts:
        ...
        temperature = forecast['main'].get('temp', 'N/A')  # Temperature in Celsius
        feels_like = forecast['main'].get('feels_like', 'N/A')  # Feels like temperature in Celsius

        # Extracting the minimum and maximum temperature
        temp_min = forecast['main'].get('temp_min', 'N/A')
        temp_max = forecast['main'].get('temp_max', 'N/A')

        # Extracting the humidity
        humidity = forecast['main'].get('humidity', 'N/A')  # Humidity in percentage

        # Extracting the pressure
        pressure = forecast['main'].get('pressure', 'N/A')  # Atmospheric pressure in hPa

        # Extracting the wind speed and direction
        wind_speed = forecast['wind'].get('speed', 'N/A')  # Wind speed in m/s
        wind_deg = forecast['wind'].get('deg', 'N/A')  # Wind direction in degrees

        # Extracting the weather description
        weather_description = forecast['weather'][0].get('description', 'N/A')  # Weather description

        # Extracting cloudiness
        cloudiness = forecast['clouds'].get('all', 'N/A')  # Cloudiness in percentage
        ...
        visibility = forecast.get('visibility', 'N/A')  # Visibility in meters
